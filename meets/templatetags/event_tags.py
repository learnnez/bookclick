...
def do_event_calendar(parser, token):
    ...
    try:
        ...
        #raise template.TemplateSyntaxError, "%s tag requires three arguments" % token.contents.split()[0]
        raise template.TemplateSyntaxError('Les arguments doivent ')
...
class EventCalendar(HTMLCalendar):
    ...
    def __init__(self, events):
        ...
        self.day_name = ('Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su' )
    ...
    def formatday(self, day, weekday):
        if day != 0:
            ...
            if day in self.events:
                ...
                #return self.day_cell(cssclass, '<span class="dayNumber"><a href="/booking/list/%s/%s">%d</a></span>' 
                #                                   % (self.year, self.month, day))
                return self.day_cell(cssclass, '<span class="dayNumber"><a href="%s">%d</a></span>' \
                         % (BookMeeting.get_day_url(self, self.year,'%02d' %self.month, '%02d' % day), day)) 
    ...
    def group_by_day(self, events):
        ...
        field = lambda event: event.book_date.day 
